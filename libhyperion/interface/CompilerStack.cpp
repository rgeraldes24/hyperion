...
void CompilerStack::setLibraries(std::map<std::string, util::h192> const& _libraries)
{
	if (m_stackState >= ParsedAndImported)
...
Json::Value CompilerStack::gasEstimates(std::string const& _contractName) const
{
	...
	if (zvmasm::AssemblyItems const* items = runtimeAssemblyItems(_contractName))
	{
		...
		{
			/// Exclude externally visible functions, constructor, fallback and receive zond function
