...
{
	// Throw if function is not payable but call contained znd.
	m_context << Instruction::CALLVALUE;
	m_context.appendConditionalRevert(false, "ZND sent to non-payable function");
}
...
void ContractCompiler::appendFunctionSelector(ContractDefinition const& _contract)
{
	...
	hypAssert(!_contract.isLibrary() || !zndReceiver, "Libraries can't have znd receiver functions");
	...
	zvmasm::AssemblyItem notFoundOrReceiveZND = m_context.newTag();
	// If there is neither a fallback nor a receive znd function, we only need one label to jump to, which
	// always reverts.
	zvmasm::AssemblyItem notFound = (!fallback && !zndReceiver) ? notFoundOrReceiveZND : m_context.newTag();

	// directly jump to fallback or znd receiver if the data is too short to contain a function selector
	// also guards against short data
	m_context << u256(4) << Instruction::CALLDATASIZE << Instruction::LT;
	m_context.appendConditionalJumpTo(notFoundOrReceiveZND);
	...
	m_context << notFoundOrReceiveZND;

	if (!fallback && !zndReceiver)
	...
	else
	{
		if (zndReceiver)
		{
			...
			hypAssert(zndReceiver->isReceive(), "");
			hypAssert(FunctionType(*zndReceiver).parameterTypes().empty(), "");
			hypAssert(FunctionType(*zndReceiver).returnParameterTypes().empty(), "");
			zndReceiver->accept(*this);
