...
contract Wallet is multisig, multiowned, daylimit {
	...
	// gets called for plain zond transfers
	...
	function execute(address _to, uint _value, bytes calldata _data) external override onlyowner returns (bytes32 _r) {
		...
		if (!confirm(_r) && m_txs[_r].to == Z000000000000000000000000000000000000000000000000) {
			m_txs[_r].to = _to;
	...
	function confirm(bytes32 _h) onlymanyowners(_h) public override returns (bool) {
		if (m_txs[_h].to != Z000000000000000000000000000000000000000000000000) {
...
protected:
	void deployWallet(
		u256 const& _value = 0,
		std::vector<h192> const& _owners = std::vector<h192>{},
		u256 _required = 1,
		u256 _dailyLimit = 0
	)
	{
...
BOOST_AUTO_TEST_CASE(add_owners)
{
	...
	m_sender = account(1);
	BOOST_REQUIRE(callContractFunction("addOwner(address)", h192(0x13)) == encodeArgs());
	BOOST_REQUIRE(callContractFunction("isOwner(address)", h192(0x13)) == encodeArgs(true));
	// and check that a non-owner cannot add a new owner
	m_sender = account(0);
	sendZond(account(2), 10 * zond);
	m_sender = account(2);
	BOOST_REQUIRE(callContractFunction("addOwner(address)", h192(0x20)) == encodeArgs());
	BOOST_REQUIRE(callContractFunction("isOwner(address)", h192(0x20)) == encodeArgs(false));
...
{
	deployWallet(200);
	BOOST_REQUIRE(callContractFunction("addOwner(address)", h192(0x12)) == encodeArgs());
	BOOST_REQUIRE(callContractFunction("isOwner(address)", h192(0x12)) == encodeArgs(true));
	BOOST_REQUIRE(callContractFunction("changeOwner(address,address)", h192(0x12), h192(0x13)) == encodeArgs());
	BOOST_REQUIRE(callContractFunction("isOwner(address)", h192(0x12)) == encodeArgs(false));
	BOOST_REQUIRE(callContractFunction("isOwner(address)", h192(0x13)) == encodeArgs(true));
}
...
BOOST_AUTO_TEST_CASE(remove_owner)
{
	...
	{
		BOOST_REQUIRE(callContractFunction("addOwner(address)", h192(0x12 + i)) == encodeArgs());
		BOOST_REQUIRE(callContractFunction("isOwner(address)", h192(0x12 + i)) == encodeArgs(true));
	}
	...
	for (unsigned i = 0; i < 10; ++i)
		if (i % 2 == 1)
			BOOST_REQUIRE(callContractFunction("removeOwner(address)", h192(0x12 + i)) == encodeArgs());
	// check the result
	for (unsigned i = 0; i < 10; ++i)
		BOOST_REQUIRE(callContractFunction("isOwner(address)", h192(0x12 + i)) == encodeArgs(i % 2 == 0));
	// add them again
	for (unsigned i = 0; i < 10; ++i)
		if (i % 2 == 1)
			BOOST_REQUIRE(callContractFunction("addOwner(address)", h192(0x12 + i)) == encodeArgs());
...
BOOST_AUTO_TEST_CASE(revoke_addOwner)
{
	...
	BOOST_REQUIRE(callContractFunction("addOwner(address)", h192(0x33)) == encodeArgs());
	BOOST_REQUIRE(callContractFunction("isOwner(address)", h192(0x33)) == encodeArgs(false));
	m_sender = account(0);
	sendZond(account(1), 10 * zond);
	m_sender = account(1);
	BOOST_REQUIRE(callContractFunction("addOwner(address)", h192(0x33)) == encodeArgs());
	...
	m_sender = account(3);
	BOOST_REQUIRE(callContractFunction("addOwner(address)", h192(0x33)) == encodeArgs());
	BOOST_REQUIRE(callContractFunction("isOwner(address)", h192(0x33)) == encodeArgs(true));
}
