...
contract GlobalRegistrar is Registrar, AuctionSystem {
	...
	function onAuctionEnd(string memory _name) internal override {
		...
		emit Changed(_name);
		if (previousOwner != Z000000000000000000000000000000000000000000000000) {
	...
	function reserve(string calldata _name) external payable {
		...
		if (needAuction)
		{
			...
			Record storage record = m_toRecord[_name];
			if (record.owner != Z000000000000000000000000000000000000000000000000)
				revert();
...
protected:
	...
	public:
		...
		h192 owner(std::string const& _name)
		{
			return callStringReturnsAddress("owner", _name);
		}
		void setAddress(std::string const& _name, h192 const& _address, bool _primary)
		{
			callStringAddressBool("setAddress", _name, _address, _primary);
		}
		h192 addr(std::string const& _name)
		{
			return callStringReturnsAddress("addr", _name);
		}
		std::string name(h192 const& _addr)
		{
			return callAddressReturnsString("name", _addr);
		}
		void setSubRegistrar(std::string const& _name, h192 const& _address)
		{
			callStringAddress("setSubRegistrar", _name, _address);
		}
		h192 subRegistrar(std::string const& _name)
		{
		...
		void transfer(std::string const& _name, h192 const& _target)
		{
...
BOOST_AUTO_TEST_CASE(reserve)
{
	...
	// should not work
	registrar.reserve("");
	BOOST_CHECK_EQUAL(registrar.owner(""), h192{});
...
BOOST_AUTO_TEST_CASE(properties)
{
	...
	for (std::string const& name: names)
	{
		...
		registrar.setAddress(name, h192(addr), true);
		BOOST_CHECK_EQUAL(registrar.addr(name), h192(addr));
		registrar.setSubRegistrar(name, h192(addr + 20));
		BOOST_CHECK_EQUAL(registrar.subRegistrar(name), h192(addr + 20));
		...
		BOOST_CHECK_EQUAL(registrar.owner(name), sender);
		registrar.setAddress(name, h192(addr + 1), true);
		BOOST_CHECK_EQUAL(registrar.addr(name), h192(addr));
		registrar.setSubRegistrar(name, h192(addr + 20 + 1));
		BOOST_CHECK_EQUAL(registrar.subRegistrar(name), h192(addr + 20));
...
BOOST_AUTO_TEST_CASE(transfer)
{
	...
	registrar.transfer(name, h192(555));
	BOOST_CHECK_EQUAL(registrar.owner(name), h192(555));
...
BOOST_AUTO_TEST_CASE(disown)
{
	...
	registrar.reserve(name);
	registrar.setContent(name, h256(u256(123)));
	registrar.setAddress(name, h192(124), true);
	registrar.setSubRegistrar(name, h192(125));
	BOOST_CHECK_EQUAL(registrar.name(h192(124)), name);
	...
	m_sender = account(0);
	registrar.disown(name);
	BOOST_CHECK_EQUAL(registrar.owner(name), h192());
	BOOST_CHECK_EQUAL(registrar.addr(name), h192());
	BOOST_CHECK_EQUAL(registrar.subRegistrar(name), h192());
	BOOST_CHECK_EQUAL(registrar.content(name), h256());
	BOOST_CHECK_EQUAL(registrar.name(h192(124)), "");
}
