...
Either add ``--libraries "file.hyp:Math=Z123456789012345678901234567890123456789012345678 file.hyp:Heap=ZabCD567890123456789012345678901234567890"`` to your command to provide an address for each library (use commas or spaces as separators) or store the string in a file (one library per line) and run ``hypc`` using ``--libraries fileName``.

.. note::
    Starting Hyperion 0.8.1 accepts ``=`` as separator between library and address, and ``:`` as a separator is deprecated. It will be removed in the future. Currently ``--libraries "file.hyp:Math:Z123456789012345678901234567890123456789012345678 file.hyp:Heap:ZabCD567890123456789012345678901234567890"`` will work too.
