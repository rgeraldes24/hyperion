...
function zeppelin_test
{
    ...
    sed -i "s|it(\('calling upgradeTo from a contract that is not an ZRC1967 proxy\)|it.skip(\1|g" test/proxy/utils/UUPSUpgradeable.test.js
    sed -i "s|it(\('calling upgradeToAndCall from a contract that is not an ZRC1967 proxy\)|it.skip(\1|g" test/proxy/utils/UUPSUpgradeable.test.js
