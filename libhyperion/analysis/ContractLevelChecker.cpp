	...
	for (FunctionDefinition const* function: _contract.definedFunctions())
	{
		...
		if (function->isReceive())
		{
			if (function->libraryFunction())
				m_errorReporter.declarationError(4549_error, function->location(), "Libraries cannot have receive znd functions.");

			if (function->stateMutability() != StateMutability::Payable)
				m_errorReporter.declarationError(
					7793_error,
					function->location(),
					"Receive znd function must be payable, but is \"" +
					stateMutabilityToString(function->stateMutability()) +
					"\"."
				);
			if (function->visibility() != Visibility::External)
				m_errorReporter.declarationError(4095_error, function->location(), "Receive znd function must be defined as \"external\".");

			if (!function->returnParameters().empty())
				m_errorReporter.fatalDeclarationError(6899_error, function->returnParameterList()->location(), "Receive znd function cannot return values.");
			if (!function->parameters().empty())
				m_errorReporter.fatalDeclarationError(6857_error, function->parameterList().location(), "Receive znd function cannot take parameters.");
		}
	}
}
	...
	if (auto const* fallback = _contract.fallbackFunction())
		if (fallback->isPayable() && !_contract.interfaceFunctionList().empty() && !_contract.receiveFunction())
			m_errorReporter.warning(
				3628_error,
				_contract.location(),
				"This contract has a payable fallback function, but no receive znd function. Consider adding a receive znd function.",
