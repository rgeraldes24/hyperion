...
static_assert(QRVMC_LATEST_STABLE_REVISION <= QRVMC_MAX_REVISION,
              "latest stable revision ill-defined");

/// QRVMC C++ API - wrappers and bindings for C++
/// @ingroup cpp
namespace qrvmc
{
...
struct address : qrvmc_address
{
    ...
    constexpr address(qrvmc_address init = {}) noexcept : qrvmc_address{init} {}
...
inline const char* to_string(qrvmc_status_code status_code) noexcept
{
    return qrvmc_status_code_to_string(status_code);
}

/// @copydoc qrvmc_revision_to_string
inline const char* to_string(qrvmc_revision rev) noexcept
{
...
public:
    ...
    explicit Result(qrvmc_status_code _status_code,
                    int64_t _gas_left,
                    int64_t _gas_refund,
    ...
    /// @param _gas_left     The amount of gas left.
    /// @param _gas_refund   The amount of refunded gas.
    explicit Result(qrvmc_status_code _status_code = QRVMC_INTERNAL_ERROR,
                    int64_t _gas_left = 0,
    ...
    explicit Result(qrvmc_status_code _status_code,
                    int64_t _gas_left,
                    int64_t _gas_refund,
                    const qrvmc_address& _create_address) noexcept
      : qrvmc_result{make_result(_status_code, _gas_left, _gas_refund, nullptr, 0)}
    {
...
/// The QRVMC Host interface
class HostInterface
{
public:
    virtual ~HostInterface() noexcept = default;

    /// @copydoc qrvmc_host_interface::account_exists
    ...
    /// @copydoc qrvmc_host_interface::call
    virtual Result call(const qrvmc_message& msg) noexcept = 0;

    /// @copydoc qrvmc_host_interface::get_tx_context
...
/// Wrapper around QRVMC host context / host interface.
///
/// To be used by VM implementations as better alternative to using ::qrvmc_host_context directly.
class HostContext : public HostInterface
{
    const qrvmc_host_interface* host = nullptr;
    qrvmc_host_context* context = nullptr;

public:
    /// Default constructor for null Host context.
    HostContext() = default;

    /// Constructor from the QRVMC Host primitives.
    ...
    HostContext(const qrvmc_host_interface& interface, qrvmc_host_context* ctx) noexcept
    ...
    Result call(const qrvmc_message& message) noexcept final
    {
...
///
/// When implementing QRVMC Host, you can directly inherit from the qrvmc::Host class.
/// This way your implementation will be simpler by avoiding manual handling
/// of the ::qrvmc_host_context and the ::qrvmc_host_interface.
...
public:
    ...
    static const qrvmc_host_interface& get_interface() noexcept;
    ...
    static DerivedClass* from_context(qrvmc_host_context* context) noexcept
    {
...
/// @copybrief qrvmc_vm
///
/// This is a RAII wrapper for qrvmc_vm, and object of this type
...
public:
    ...
    /// Converting constructor from qrvmc_vm.
    ...
    /// Checks whenever the VM instance is ABI compatible with the current QRVMC API.
    bool is_abi_compatible() const noexcept { return m_instance->abi_version == QRVMC_ABI_VERSION; }

    /// @copydoc qrvmc_vm::name
    char const* name() const noexcept { return m_instance->name; }

    /// @copydoc qrvmc_vm::version
    char const* version() const noexcept { return m_instance->version; }

    /// Checks if the VM has the given capability.
    bool has_capability(qrvmc_capabilities capability) const noexcept
    {
        return (get_capabilities() & static_cast<qrvmc_capabilities_flagset>(capability)) != 0;
    }

    /// @copydoc qrvmc_vm::get_capabilities
    qrvmc_capabilities_flagset get_capabilities() const noexcept
    {
        return m_instance->get_capabilities(m_instance);
    }

    /// @copydoc qrvmc_set_option()
    qrvmc_set_option_result set_option(const char name[], const char value[]) noexcept
    {
        return qrvmc_set_option(m_instance, name, value);
    }

    /// @copydoc qrvmc_execute()
    Result execute(const qrvmc_host_interface& host,
                   qrvmc_host_context* ctx,
                   qrvmc_revision rev,
    ...
    /// Convenient variant of the VM::execute() that takes reference to qrvmc::Host class.
    ...
    ///
    /// The same as
    /// execute(const qrvmc_host_interface&, qrvmc_host_context*, qrvmc_revision,
    ///         const qrvmc_message&, const uint8_t*, size_t),
    ...
    Result execute(qrvmc_revision rev,
                   const qrvmc_message& msg,
                   const uint8_t* code,
                   size_t code_size) noexcept
    {
...
inline qrvmc_result call(qrvmc_host_context* h, const qrvmc_message* msg) noexcept
{
    return Host::from_context(h)->call(*msg).release_raw();
}

inline qrvmc_tx_context get_tx_context(qrvmc_host_context* h) noexcept
{
