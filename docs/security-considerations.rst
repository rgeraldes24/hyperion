...
and any transfer of ZND hands over control to that contract (B).
...
.. code-block:: hyperion
    ...
    contract Fund {
        /// @dev Mapping of znd shares of the contract.
...
ZND transfer can always include code execution,
so the recipient could be a contract that calls back into ``withdraw``.
This would let it get multiple refunds and, basically, retrieve all the ZND in the contract.
...
Note that reentrancy is not only an effect of ZND transfer
...
Sending and Receiving ZND
===========================

- Neither contracts nor "external accounts" are currently able to prevent someone from sending them ZND.
  Contracts can react on and reject a regular transfer, but there are ways to move ZND without creating a message call.
  One way is to simply "mine to" the contract address.

- If a contract receives ZND (without a function being called), either the :ref:`receive ZND <receive-znd-function>`
  or the :ref:`fallback <fallback-function>` function is executed.
  If it does not have a ``receive`` nor a ``fallback`` function, the ZND will be rejected (by throwing an exception).
  ...
  To be sure that your contract can receive ZND in that way, check the gas requirements of the receive and fallback functions
...
- If you want to send ZND using ``address.transfer``, there are certain details to be aware of:
  ...
  2. Sending ZND can fail due to the call depth going above 1024. Since the
     ...
     contract using a pattern where the recipient can withdraw ZND instead.
  3. Sending ZND can also fail because the execution of the recipient
...
Now someone tricks you into sending ZND to the address of this attack wallet:
...
Restrict the Amount of ZND
============================

Restrict the amount of ZND (or other tokens) that can be stored in a smart contract.
If your source code, the compiler or the platform has a bug, these funds may be lost.
If you want to limit your loss, limit the amount of ZND.
...
(who called the function, are the arguments in range, did they send enough ZND,
...
You can add a function in your smart contract that performs some self-checks like "Has any ZND leaked?",
...
or just converts the contract into a simple "give me back my ZND" contract.
