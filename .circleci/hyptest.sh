...
IFS=" " read -r -a HYPTEST_FLAGS <<< "$HYPTEST_FLAGS"
...
do
    ...
    echo "Running ${REPODIR}/build/test/hyptest ${BOOST_TEST_ARGS_RUN[*]} -- ${HYPTEST_ARGS[*]}"

    "${REPODIR}/build/test/hyptest" -l test_suite "${BOOST_TEST_ARGS_RUN[@]}" -- "${HYPTEST_ARGS[@]}" "${BATCH_ARGS[@]}" &
    PIDs+=($!)
done
